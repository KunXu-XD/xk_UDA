...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.liner1 = nn.Linear(224, 160)
        self.liner2 = nn.Linear(160, 80)
        self.liner3 = nn.Linear(80, 2)

    def forward(self, x):
        x = x.view(-1, 224)
        x = F.leaky_relu_(self.liner1(x))
        x = F.leaky_relu_(self.liner2(x))
        d_label = F.sigmoid(self.liner3(x))
        return d_label

class DANN(nn.Module):
    def __init__(self):
        super(DANN, self).__init__()
        self.feature = CNN_Extractor()
        self.classifier = classifier()
        self.domain_Discriminator = Discriminator()

    def forward(self, x, alpha, domain_label):
        feature = self.feature(x)
        Fea,class_output = self.classifier(feature)
        domain_output = self.get_adversarial_result(feature, alpha, domain_label)

        return Fea, class_output, domain_output

    def get_adversarial_result(self, x, alpha, domain_label):
        loss_fn = nn.CrossEntropyLoss()
        x = adv_layer.ReverseLayerF.apply(x, alpha)
        domain_pred = self.domain_Discriminator(x)
        loss_adv = loss_fn(domain_pred.squeeze(), domain_label)
...
def evaluate(model, eval_loader, alpha, domain_label):
    model.eval()
    correct = 0
    total = 0
    with torch.no_grad():
        ...
        for step, (inputs, labels) in enumerate(eval_loader):
            ...
            fea,cset_logit,_ = model(inputs,alpha, domain_label)
...
def train(Epoch,model,data_src,data_tar,gpus,cuda_gpu):
    ...
    for epoch in range(Epoch):
        ...
        #     _, (im_source, label_source) = list_src[step]
            ...
            "反转层参数设置"
            len_dataloader = len(data_src)
            p = float(t + epoch * len_dataloader) / Epoch / len_dataloader
            t += 1
            alpha = 2. / (1. + np.exp(-10 * p)) - 1
            alpha = alpha / math.pow((1 + 10 * epoch / Epoch), 0.75)
            "损失计算"
            domain_src = torch.zeros(len(im_source)).long()
            s_fea, s_cls_logit, loss_adv_source = model(im_source, alpha=alpha, domain_label=domain_src)
            domain_tar = torch.ones(len(im_target)).long()
            t_fea, t_cls_logit, loss_adv_target = model(im_target, alpha=alpha, domain_label=domain_tar)
            source_c_loss = F.cross_entropy(s_cls_logit, label_source)
            loss_all = source_c_loss + loss_adv_source  + loss_adv_target
            "反向传播"
            optimizer.zero_grad()
            loss_all.backward()
            optimizer.step()
            metric = {'source_cset_loss': source_c_loss.item()}
            metric['loss_adv_source'] = loss_adv_source.item()
            metric['loss_adv_target'] = loss_adv_source.item()

        _,_,_,Acc_train = evaluate(model,data_src,alpha=alpha, domain_label=domain_src)
        Fea_tar, predictions_tar, label_real, Acc_test = evaluate(model, data_tar,alpha=alpha, domain_label=domain_tar)
        if epoch<1:
        ...
        item_pr = 'Epoch: [{}/{}], source_cset_loss: {:.6f},loss_adv_source: {:.6f}, loss_adv_target: {:.6f}, Acc_train: {:.6f},Acc_test: {:.6f}, epoch_time: {:.6f}'.format(
        epoch, Epoch,  metric['source_cset_loss'], metric['loss_adv_source'], metric['loss_adv_target'],Acc_train, Acc_test, epoch_time)
        print(item_pr)
    return alpha,domain_src, Fea_tar_best, predictions_tar_best, label_real_best, Acc_tar_best, model
...
if __name__ == '__main__':
    ...
    model1 = DANN()
    # 训练模型
    alpha,domain_src,Fea_tar, predictions_tar, label_real, Acc_tar, model = train(Epoch=50, model=model1, data_src=data_src, data_tar=data_tar, gpus=gpus, cuda_gpu=cuda_gpu)

    # 训练集精度
    Fea_train, predictions_train,_, Acc_train = evaluate(model, data_src,alpha,domain_src)
    # 测试集精度
    ...
    io.savemat('DANN_OPDA.mat', {'Fea': Fea_tar, 'label': label_real})
