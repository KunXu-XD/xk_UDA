...
def train(Epoch,model,data_src,data_tar,gpus,cuda_gpu):
    ...
    for epoch in range(Epoch):
        ...
        target_memory_bank = torch.zeros(120, 224)
...
if __name__ == '__main__':
    ...
    data_src = Data.DataLoader(dataset=Class_src, batch_size=120, sampler=sampler_src,drop_last=True)#, sampler=sampler_src
    data_tar = Data.DataLoader(dataset=Class_tar, batch_size=120, sampler=sampler_tar,drop_last=True)#, sampler=sampler_tar
    "#######——————————模型训练——————————#######"
    #cuda运行
    ...
    io.savemat('MLnet_OPDA.mat', {'Fea': Fea_tar, 'label': label_real})
